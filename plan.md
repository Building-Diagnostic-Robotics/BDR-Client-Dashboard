...
- Retain Audit-table records for three years from `occurredAt`; a retention TTL may remove them only after that period and is not used for authorization. Enable deletion protection, point-in-time recovery, KMS encryption, and CloudTrail data events for Audit-table writes, updates, and deletes.
- Export the Audit table weekly to a dedicated private audit-archive S3 bucket with a separate KMS key, versioning, Block Public Access, and default three-year S3 Object Lock governance retention. Only a separately controlled audit-reader role can read exports; application roles have no archive access.
- Alert on failed audit writes, failed weekly exports, and any attempted Audit-table `UpdateItem`, `DeleteItem`, or `BatchWriteItem`. Audit-export jobs use their own least-privilege role.
...
- Audit-event duplicate keys, updates, deletes, and batch overwrites are rejected. Audit writes participate in the related DynamoDB transaction where possible; records and weekly locked exports remain available for the three-year retention period, and failed writes or exports raise alarms.
...
8. Add append-only auditing, three-year retention, weekly locked audit exports, throttling, security headers, PITR, CloudTrail write/delete events, and alarms.
